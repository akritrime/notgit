...
impl Display for NGitError {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		match self {
			...
			NGitError::MissingArgument(cmd, ty) => write!(f, "missing '{}' argument for '{}' command", ty, cmd),
			...
			NGitError::UnexpectedDataType(expected, received) => write!(f, "expected {} data type, received {}", expected, received),
			NGitError::InvalidObject(h) => write!(f, "corrupted objected at {}", h),
			// NGitError::InvalidFileRead(s) => write!(f, "unable to read file at {}", s)
			NGitError::NoArgumentExpected => write!(f, "no argument expected by the specified command"),
			NGitError::InvalidCommit(oid) => write!(f, "commit {} is invalid", oid),
			// NGitError::MissingHead => write!(f, "missing HEAD"),
			NGitError::Unresolvable(s) => write!(f, "expected a SHA1 digest or a reference; received {}", s),
